...
mapping(uint256 => Rent) public landRent;

function landExists(uint256 landId) public view returns(bool) {
    return _exists(landId);
}
function setLandRent(uint256 landId, uint256 rentAmount, uint256 rentDuration) public  {
    require(landExists(landId), "Land does not exist");
    int x;
    int y;
    (x,y)= _decodeTokenId(landId);
    address owner = _ownerOfLand(x,y);
    require(owner == msg.sender,"Not an land owner");
    landRent[landId] = Rent(rentAmount, rentDuration, msg.sender);
    _doTransferFrom(msg.sender,address(this),landId,"",false);
}

function getLandRent(uint256 landId) public view returns (uint256, uint256, address) {
    require(landExists(landId), "Land does not exist");
    Rent memory rent = landRent[landId];
    return (rent.amount, rent.duration, rent.owner);
}

function payLandRent(uint256 landId) public payable {
    require(landExists(landId), "Land does not exist");
    
    Rent memory rent = landRent[landId];
    require(rent.duration >= block.timestamp);
   // require(msg.sender == rent.ow, "You are not the tenant of this land");
    //require(msg.value == rent.amount, "Incorrect rent amount");
    // int x;
    // int y;
    // (x,y)= _decodeTokenId(landId);
    // address owner = _ownerOfLand(x,y);
    IERC20(dimeToken).transferFrom(msg.sender,rent.owner,rent.amount);


  
}

function withdrawLand(uint256 landId) public {
    require(landExists(landId), "Land does not exist");
    Rent memory rent = landRent[landId];
    require(msg.sender == rent.owner, "You are not the tenant of this land");
    require(rent.duration < block.timestamp, "Rent duration has not expired");

    // Transfer land ownership to tenant
    _doTransferFrom(address(this),msg.sender,landId,"",false);
    // Clear rent information
    delete landRent[landId];
}
