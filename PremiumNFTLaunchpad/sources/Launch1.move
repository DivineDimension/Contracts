module Launch1::Launch1 {
    ...
    public entry fun participate<C>(source_account: &signer,owner_addr: address, collection_name: String,description: String,collection_uri: String, token_uri: String,percentageaddress:address) acquires Cal,Mydeposits {
        ...
        assert!(value.limit >= value.count, error::invalid_state(LIMITEXPIRED));
    ...
    public entry fun phaseReseter(module_owner: &signer,stratTime:u64,endTime:u64,price:u64,limit:u64) acquires Cal  {
        // Assert that the module owner signer is the owner of this module.
        assert!(signer::address_of(module_owner) == @Launch1, error::permission_denied(ENOT_AUTHORIZED));
        let value = borrow_global_mut<Cal>(signer::address_of(module_owner));
        value.start_time = stratTime;
        value.end_time = endTime;
        value.limit = limit;
        value.price = price;
        value.phase = value.phase + 1;
